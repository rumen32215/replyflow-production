...
| **Relationship** | "Remembers people, not just jobs — previous problems, promises, preferences." | `conversations.communication_preference` (one field), service history and outstanding-work cards on the customer detail page (Task 2.3). No recognition of *meaning* (a returning customer, a repeat emergency caller) — that's the Acknowledgement layer, designed but not built (see the architecture design review, 2026-08-01). | Partial |
