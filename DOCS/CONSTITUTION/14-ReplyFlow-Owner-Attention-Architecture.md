...
**A deployment-infrastructure note, not an architectural one:** today's hosting tier (Vercel Hobby) only supports a daily cron, so the deployed cadence is currently daily rather than the minute-level tick this document's Now/Today distinction is designed for. This is an operational deployment limitation, not a limitation of the design — §0–§9 above describe the intended behaviour unchanged, and should not be redesigned around today's hosting tier. Tracked as a pilot readiness prerequisite (`ReplyFlow-Go-No-Go-Review.md`), the same category as SUPPORT_EMAIL/ADMIN_EMAILS/RESEND_API_KEY, not as a Constitution change.
