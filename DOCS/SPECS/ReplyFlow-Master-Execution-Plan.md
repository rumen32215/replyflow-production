...
### The silent-return gap inside the reply pipeline itself — closed and live-confirmed

Fourth task under Architecture-Led Development, applying the founder's "will ReplyFlow know about it?" principle one level deeper than the previous `app/api` route audit — to every silent `return` inside `lib/reply-engine/generate-reply.ts` itself, the single most consequential file in the product. Found one real, reachable gap and ruled out the rest: the `if (!conversation) return;` lookup near the top of the pipeline wrote nothing and recorded nothing on failure. Because a `return` isn't a thrown exception, this never reached the existing `reply-engine.pipeline_failure` critical event either — a genuine blind spot in that event's own coverage, and a shape (no draft, no logged exception) that matches exactly what the SLI doc's 4 still-unexplained silent-drop cases look like. Every other silent-return branch in the file was checked individually and confirmed to be either an intentional, documented business rule (idempotency guard, not-ready-to-act-alone gate, spam gets no reply) or provably unreachable given `SafetyTag`'s closed type union — not gaps.

**Shipped:** the branch now calls `recordErrorEvent` (`critical`, `reply-engine.conversation_not_found`) before returning.
...
### The owner should eventually know — Organise Checkpoint v1.1

Sixth task under Architecture-Led Development, following the founder's own further refinement of the standing principle: even when a failure is handled gracefully and the immediate experience isn't interrupted, ask whether the business owner should eventually be told. Investigated whether today's earlier fixes already answer this — checked each one's real frontend caller (`preparing-receptionist.tsx`, `work-card-detail.tsx`, `whatsapp-embedded-signup.tsx`) and confirmed all three already surface their failure to the owner immediately, in the moment, via a toast/error state. No gap there. The real gap is the reply-engine's own background failures: they run via `waitUntil` with no owner session open at all. `recordErrorEvent` makes them durably recorded, but the only readers of `error_events` anywhere in the app are the founder's own admin pages (`app/admin/**`) — the owner, whose actual customer this was, never finds out, not even eventually.

**Shipped:** a second rule on the Organise Checkpoint (`lib/brain/organise.ts`), the exact extension point it was built for. `unrepliedMessageRule` surfaces through the same existing Observation/InsightList mechanism already on Front Desk — no new UI surface, no new component. Unlike v1's original rule (deliberately the only one, pending pilot data for anything heuristic), this rule doesn't need that same gating: it surfaces an already-confirmed fact (a real `error_events` critical row already recorded that a specific customer message got zero reply), not a guess about business process. Deliberately narrow scope: only the two sources that mean a customer got literally nothing (`reply-engine.pipeline_failure`, `reply-engine.conversation_not_found`), only `critical` severity, bounded to the same 7-day window every other "recent" Front Desk query already uses. `reply-engine.send_failed` was deliberately excluded — that failure is already surfaced to the owner in real time when they click Approve, so it doesn't share this specific gap.

**Verification, honestly scoped:** tsc/lint/131 tests (3 new)/build all pass; confirmed the deploy is live against production. Couldn't render the authenticated Front Desk page directly (no login credentials in this environment, consistent with every other owner-facing verification today) — instead directly verified the exact new Supabase query (jsonb `context` extraction, the `source` filter, the date window) against real production data: inserted a real test `error_events` row tied to a real conversation, confirmed the query correctly found and resolved it to the right conversation id, then cleaned up. Combined with the unit tests proving the pure rule logic, this gives strong end-to-end confidence without claiming a render-level check that wasn't actually possible here.
