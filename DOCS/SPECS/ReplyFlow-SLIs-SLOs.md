...
**Update, 2026-08-01: now measurable, with a caveat.** Every real inbound WhatsApp message already reaches `app/api/webhooks/whatsapp/route.ts` regardless of whether our own access token can still call the Graph API — that makes it a real, always-hit chokepoint for checking `token_expires_at`, with no new polling infrastructure needed. The handler now runs `describeConnectionHealth()` (already tested, unchanged) on every real inbound message and records a `critical` `error_events` row (`webhook.whatsapp_token_expired`) once expired, or a `warning` (`webhook.whatsapp_token_expiring_soon`) inside the existing 3-day warning window — deduplicated (1h/24h windows) so a sustained outage produces one alert, not one per message. This reuses 1.3's existing `notifyCriticalIncident()` unchanged, so once a founder sets `INCIDENT_ALERT_WEBHOOK_URL`, this starts actively alerting for free. Verified against real production: `token_expires_at` set to the past on the real QA connection, a real signed webhook POST, confirmed exactly one `critical` row written with the correct message, confirmed a second inbound message within the dedup window did not duplicate it, then fully restored and cleaned up.

**The honest caveat:** detection latency is now bounded by *real customer contact*, not a fixed interval — a business with no inbound traffic while disconnected won't get detected until the next real message arrives (or an owner opens the app, as before). The original 15-minute-regardless-of-silence target still needs an active scheduled check, which needs a decision this fix deliberately avoided requiring: Vercel Cron on this project's current plan tier only runs once/day, so a true interval check needs a plan decision, which is a founder call, not an engineering one. This closes the "currently unmeasurable, zero detection" gap with zero new infra or founder decisions; the interval-based version remains a distinct, smaller, later piece of work.
...
**Update, 2026-08-02 — a real, confirmed-reachable mechanism found for exactly this shape of gap (not proven as the historical cause, but closed either way).** Applying the founder's "will ReplyFlow know about it?" principle one level deeper than `app/api` catch blocks — to every silent `return` inside `generateReplyForMessage` itself — found that `if (!conversation) return;` (the conversation lookup right at the top of the pipeline) wrote no `reply_drafts` row and called no `recordErrorEvent`. Critically, a `return` is not a thrown exception, so this never reached the existing `reply-engine.pipeline_failure` event either — a real gap in that event's own coverage, exactly matching why the 4 remaining cases above show *no logged exception at all* despite the pipeline "running successfully." **This is not a theory** — it was deliberately reproduced live against real production (a real webhook POST, racing a delete of the just-created conversation row against the pipeline's own async read) and confirmed to fire correctly with the right severity/source/context. The race only won on the second of two genuine attempts, meaning the real window is very tight — consistent with this explaining a small number of historical cases, not being a frequent occurrence. **Closed**: this branch now calls `recordErrorEvent` (`critical`, `reply-engine.conversation_not_found`) before returning — see `lib/reply-engine/generate-reply.ts`. Whether this was *the* actual cause of the 4 remaining historical cases can't be proven retroactively (no audit trail of what a conversation row looked like at that exact past moment) — stated honestly as consistent-with, not confirmed-as, per this project's own evidence discipline. Any future occurrence is now diagnosable instead of invisible, which is the real, honest close here.
