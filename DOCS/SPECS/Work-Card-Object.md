...
The object, schema, relationships, and the automatic-field pipeline are built (migration `0013_work_cards.sql`, `lib/work-card.ts`, wired into `components/dashboard/conversations/conversation-story.tsx` and `app/api/work-cards/[id]/approve/route.ts`).

**The dedicated page is now built too (Master Execution Plan 2.1):** `/dashboard/work-cards/[id]` — every field in the table above except Photos, full editing, address confirmation (soft warning, exactly as resolved in §5), and status transitions across the whole lifecycle. Every existing reference to a Work Card (Front Desk's Today's Work and Needs Your Attention, the Customer page's service history) now points here instead of the parent conversation.

**Still not built, deliberately:** Photos — the `messages` table has no media/attachment storage at all yet, so this needs its own decision before it can be implemented, not a guess bolted onto this pass. A dedicated Work Cards list/nav destination — `06-Experience-Architecture.md` §2 names this as intentionally deferred, not a gap.

**A small correction found while building the page:** the field table above lists a "completion note" as distinct from "Completed at" — this was never actually added to the schema (only `completed_at` exists; there is no separate `completion_note` column). Noted honestly rather than silently building a field the spec implied but the database never got. Not fixed here — the page's existing `notes` field already serves this purpose in practice; a dedicated column is a small, separate decision if it's ever genuinely needed.
