...
export const metadata: Metadata = { title: "Report — ReplyFlow" };
...
export default async function ReportDetailPage({ params }: { params: { id: string } }) {
  ...
  // 6) — status, issue, customer name, address, and job date are all
  // live-fetched from the Work Card here, never job_docs' own (now
  // unused) customer_name/job_address/job_date columns, which is
  // exactly what used to drift out of sync with what the Work Card
  // actually said. Phase 3 step 10 — customer_phone gets the same
  // treatment: the linked Conversation is the live source, job_docs'
  // own copy (P3.10: no longer written for new records at all) only
  // covers the rare pre-cleanup row that predates this.
  const [{ data: workCard }, { data: conversation }] = await Promise.all([
    jobDoc.work_card_id
      ? supabase
          .from("work_cards")
          .select("status, issue, customer_name, address, scheduled_for, completed_at, episode_id")
          .eq("id", jobDoc.work_card_id)
          .maybeSingle()
      : Promise.resolve({ data: null }),
    jobDoc.conversation_id
      ? supabase.from("conversations").select("customer_phone").eq("id", jobDoc.conversation_id).maybeSingle()
      : Promise.resolve({ data: null }),
  ]);
  const isJobCompleted = workCard?.status === "completed";
  const issueReported = workCard?.issue ?? null;
  const customerName = workCard?.customer_name ?? jobDoc.customer_name;
  const jobAddress = workCard?.address ?? jobDoc.job_address;
  const jobDate = workCard ? workCard.completed_at ?? workCard.scheduled_for : jobDoc.job_date;
  const customerPhone = conversation?.customer_phone ?? jobDoc.customer_phone;
  ...
  // Plumber Reset Phase 3 step 7 — the Job's full, unified evidence:
  // WhatsApp photos and anything manually uploaded onto this report,
  // together (lib/job-docs/job-evidence.ts). RLS already scopes the
  // read to this owner's business via job_docs/work_cards above; the
  // service role is only needed afterward, for signed URLs into
  ...
  return (
    <div className="mx-auto max-w-[760px] space-y-6">
      <div>
        <Link
          href={jobDoc.work_card_id ? `/dashboard/work-cards/${jobDoc.work_card_id}` : "/dashboard/work-cards"}
        ...
        <div className="mt-2 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <Link
              href={`/dashboard/reports/${jobDoc.id}/preview`}
      ...
      <section className="rounded-2xl border border-border bg-card p-5">
        ...
        <dl className="mt-3 grid gap-x-6 gap-y-2 sm:grid-cols-2">
          <div>
            ...
            <dd className="text-[14px] font-medium">{customerName || "—"}</dd>
          </div>
          <div>
            <dt className="text-[11.5px] text-muted-foreground">Address</dt>
            <dd className="text-[14px] font-medium">{jobAddress || "—"}</dd>
          </div>
          <div>
            <dt className="text-[11.5px] text-muted-foreground">Phone</dt>
            <dd className="text-[14px] font-medium">{customerPhone || "—"}</dd>
          </div>
          <div>
          ...
          <div>
            ...
            <dd className="text-[14px] font-medium">
              {jobDate ? new Date(jobDate).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" }) : "—"}
            </dd>
          </div>
        </dl>
      </section>
      ...
      <section className="rounded-2xl border border-border bg-card p-5">
        <h2 className="mb-4 text-[13px] font-bold uppercase tracking-wide text-muted-foreground">Report</h2>
