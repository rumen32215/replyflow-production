...
export function ConnectionAlert({ health }: { health: ConnectionHealth }) {
  if (health.status !== "expiring_soon" && health.status !== "expired") return null;
      ...
      <Link href="/dashboard/whatsapp" className="group block">
        <div
          className={`flex items-center gap-3 rounded-2xl border-l-4 p-4 pl-3.5 transition-shadow group-hover:shadow-sm ${
            expired ? "border-destructive bg-destructive/[0.06]" : "border-attention bg-attention/[0.06]"
          }`}
        >
          <div
            className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full ${
              expired ? "bg-destructive/15 text-destructive" : "bg-attention/15 text-attention"
            }`}
          >
            <AlertTriangle className="h-4 w-4" />
          </div>
          <div className="min-w-0 flex-1">
            <p className="truncate text-[14px] font-semibold">{expired ? "WhatsApp connection expired" : "WhatsApp connection expiring soon"}</p>
